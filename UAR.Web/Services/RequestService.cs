...
public class RequestService
{
    ...
    private static void AddRequestParameters(SqlCommand command, UarRequest request)
    {
        ...
        command.Parameters.AddWithValue("@ProgramAdministratorName", request.ProgramAdministratorName);
        command.Parameters.AddWithValue("@ProgramAdministratorEmail", request.ProgramAdministratorEmail);
    ...
    private static UarRequest MapRequest(SqlDataReader reader)
    {
        return new UarRequest
        {
            ...
            ProgramAdministratorName = GetStringSafe(reader, 27),
            ProgramAdministratorEmail = GetStringSafe(reader, 28),
            RdoApproverName = GetStringSafe(reader, 29),
            RdoApproverEmail = GetStringSafe(reader, 30),
            RejectionReason = GetStringSafe(reader, 31),
            EmailAccount = GetStringSafe(reader, 32),
            AccountEnableDisable = GetStringSafe(reader, 33),
            ForwardingEmailAddress = GetStringSafe(reader, 34),
            AssignedAccountUsername = GetStringSafe(reader, 35),
            Office365License = GetStringSafe(reader, 36),
            AdditionalMicrosoftProducts = GetStringSafe(reader, 37),
            CreateEmailGroups = GetStringSafe(reader, 38),
            SharePointSiteUrl = GetStringSafe(reader, 39),
            SharePointSiteDescription = GetStringSafe(reader, 40),
            ExistingEmailGroups = GetStringSafe(reader, 41),
            AdditionalSharepointAccess = GetStringSafe(reader, 42),
            NetworkSharedDrives = GetStringSafe(reader, 43),
            TopAccess = GetStringSafe(reader, 44),
            KronosAccessTypes = GetStringSafe(reader, 45),
            KronosAccessDetails = GetStringSafe(reader, 46),
            LawsonAccess = GetStringSafe(reader, 47),
            AdditionalLawsonAccess = GetStringSafe(reader, 48),
            MileageAccess = GetStringSafe(reader, 49),
            ComericaBankingAccess = GetStringSafe(reader, 50),
            TrustRepPayeeAccess = GetStringSafe(reader, 51),
            QuickbooksAccess = GetStringSafe(reader, 52),
            CoWorkerAccess = GetStringSafe(reader, 53),
            AdditionalHrFinanceAccess = GetStringSafe(reader, 54),
            OrderConnectAccess = GetStringSafe(reader, 55),
            CaminarAccess = GetStringSafe(reader, 56),
            AvatarAccess = GetStringSafe(reader, 57),
            AdverseEventSupervisorAccess = GetStringSafe(reader, 58),
            AdverseEventAdditionalProgramAccess = GetStringSafe(reader, 59),
            BusinessIntelligenceRole = GetStringSafe(reader, 60),
            AdverseEventNonEmployeeAccess = GetStringSafe(reader, 61),
            TelehealthAccess = GetStringSafe(reader, 62),
            NextTelehealthSession = GetStringSafe(reader, 63),
            AdditionalComments = GetStringSafe(reader, 64),
            PharmericaAccess = GetStringSafe(reader, 65),
            PharmericaUserRole = GetStringSafe(reader, 66),
            PharmericaUsername = GetStringSafe(reader, 67),
            CallCenterAccess = GetStringSafe(reader, 68),
            AdobeSignAccount = GetStringSafe(reader, 69),
            CopilotLicense = GetStringSafe(reader, 70),
            SmartsheetLicense = GetStringSafe(reader, 71),
            EFax = GetStringSafe(reader, 72),
            ApprovalToken = GetGuidSafe(reader, 73),
            RejectionToken = GetGuidSafe(reader, 74),
            ApprovalDecision = GetStringSafe(reader, 75),
            ApprovedOn = GetDateTimeSafe(reader, 76),
            ApprovedBy = GetStringSafe(reader, 77),
            RejectedOn = GetDateTimeSafe(reader, 78),
            RejectedBy = GetStringSafe(reader, 79)
        };
    }
